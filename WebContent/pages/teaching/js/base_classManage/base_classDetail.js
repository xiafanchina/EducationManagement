...
class_id=class_id.replace('class_id=','');//获得需要显示的班级id
$(function () {
	 $("div.buttonGather > button.modify").click(function () {
         location.href = "base_classModify.html?class_id=" + class_id;
	 })
	...
	$.ajax({
		...
		success:function(data){
			$(".td3").eq(0).text(data[0].class_id);//班级ID
			$(".td3").eq(1).text(data[0].name);//班级名称
			$(".td3").eq(2).text(data[0].lesson);//方向
			$(".td3").eq(3).text(data[0].status);//班级状态
			$(".td3").eq(4).text(data[0].lecturer);//主讲
			$(".td3").eq(5).text(data[0].assistant);//助教
			$(".td3").eq(6).text(data[0].teacher);//班主任
			$(".td3").eq(7).text(data[0].class_num);//班级人数
			$(".td3").eq(8).text(data[0].class_place);//开班地点
			$(".td3").eq(9).text(data[0].class_start);//开班时间
			$(".td3").eq(10).text(data[0].course_start);//开课时间
			$(".td3").eq(11).text(data[0].course_end);//结课时间
			$(".td3").eq(12).text(data[0].class_way);//开班方式		
		}
	})
    temp=$("#tb").clone();
    $("#tb").empty();
...
function splitPage(page){ 
	$.ajax({
		url:"../../showStuByIdSplit.do",
		type:"post",
		dataType:"json",
		data:"currentPage="+page+"&pageSize="+pageSize+"&class_id="+class_id,
		success:function(data){
			count1=data.count;
			...
			$.each(data.list,function(i,data1){
				temp1.find(".words:eq(0)").text(num);
				temp1.find(".td1:eq(1)").text(data1.id);
				temp1.find(".td1:eq(2)").text(data1.email);
				temp1.find(".td1:eq(3)").text(data1.stus_name);
				if(data1.sex=="1"){
					temp1.find(".td1:eq(4)").text("男");
				}else if(data1.sex=="2"){
					temp1.find(".td1:eq(4)").text("女");
				}
				temp1.find(".td1:eq(5)").text(data1.phone);
				temp1.find(".td1:eq(6)").text(data1.school);
				if(data1.stu_status=="1"){
					temp1.find(".td1:eq(7)").text("新入学");
				}else if(data1.stu_status=="2"){
					temp1.find(".td1:eq(7)").text("正在上课");
				}else if(data1.stu_status=="3"){
					temp1.find(".td1:eq(7)").text("停课中");
				}else if(data1.stu_status=="4"){
					temp1.find(".td1:eq(7)").text("已毕业");
				}
